...
class ParkingLotterySystem {
    constructor() {
        this.history = JSON.parse(localStorage.getItem('parkingLotteryHistory')) || [];
        
        // 初始化數字管理器
        this.lotteryManager = new NumberManager().init(
            'lottery-numbers-display', 'lottery-count', '抽籤號碼'
        );
        
        this.parkingManager = new NumberManager().init(
            'parking-numbers-display', 'parking-count', '車位號碼'
        ).setExcludeChecker((num) => this.excludeManager.includes(num));
        
        this.excludeManager = new NumberManager().init(
            'exclude-numbers-display', 'exclude-count', '排除車位',
    ...
    initializeThemeAndLanguage() {
        document.body.className = this.currentTheme === 'github' ? 'github-style' : '';
    ...
    initializeEventListeners() {
        ...
        // 抽籤和重置按鈕
        document.getElementById('start-lottery').addEventListener('click', () => this.startLottery());
        document.getElementById('reset-lottery').addEventListener('click', () => this.resetLottery());
    ...
    conductLottery(lotteryNumbers, availableSpots) {
        ...
        if (lotteryNumbers.length <= availableSpots.length) {
            ...
            selectedLotteryNumbers = this.shuffleArray([...lotteryNumbers]).slice(0, availableSpots.length);
    ...
    displayResults(results) {
        ...
        const tableHtml = Templates.createResultsTable(pageResults, 1, totalResults, totalPages, resultsPerPage);
    ...
    showHistoryDetails(historyIndex) {
        ...
        resultsContainer.innerHTML = Templates.createHistoryDetails(record, historyIndex);
    ...
    async copyCSV(historyIndex) {
        ...
        let csvContent = `抽籤結果\n抽籤時間,${timestamp.toLocaleString('zh-TW')}\n`;
    ...
    async copyParkingNumbers(historyIndex) {
        ...
        const parkingNumbers = record.results.map((pair) => pair.parkingSpot).join(',');
    ...
    saveToHistory(results) {
        ...
        localStorage.setItem('parkingLotteryHistory', JSON.stringify(this.history));
        this.loadHistory();
    }
    ...
    loadHistory() {
        ...
        const historyHtml = this.history.map((record, index) => {
            ...
            return `
                <div class="history-item" onclick="lottery.showHistoryDetails(${index})" style="cursor: pointer;">
                    ...
                    <div class="history-summary">
                        ...
                        ${record.selectedCount !== record.participants ? ` | 實際參與：${record.selectedCount}個` : ''}
                    </div>
    ...
    resetLottery() {
        if (confirm('確定要重置所有抽籤設定嗎？這將清空所有號碼設定。')) {
            document.getElementById('results-container').innerHTML = '<div class="no-results">尚未進行抽籤</div>';
            
            this.lotteryManager.numbers = [];
            this.parkingManager.numbers = [];
            this.excludeManager.numbers = [...ParkingConfig.defaultExcludedSpots];
    ...
    migrateHistoryData() {
        ...
        this.history.forEach((record) => {
            if (record.zoneResults && !record.lotteryNumbers) {
                // 轉換舊格式到新格式
                const allResults = [];
                if (record.zoneResults.aZone) allResults.push(...record.zoneResults.aZone);
                if (record.zoneResults.bZone) allResults.push(...record.zoneResults.bZone);
                
                record.results = allResults;
                record.lotteryNumbers = allResults.map(r => r.lotteryNumber);
                record.parkingRange = allResults.map(r => r.parkingSpot).join(',');
