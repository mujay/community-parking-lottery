...
一個現代化的網頁應用程式，專為社區停車位分配而設計的抽籤系統。支援多種佈景主題切換（日系風格、GitHub 風格），具備靈活的號碼範圍管理、完整的結果匯出功能，以及全站 CSS 變數化的主題一致性設計。
...
-   **統一抽籤**：所有停車位統一抽籤分配
-   **智慧配對**：自動處理抽籤號碼數量與車位數量的匹配
-   **公平隨機**：使用 Fisher-Yates 洗牌演算法確保公平性
...
-   **單個顯示**：抽籤號碼以單個標籤形式顯示，可個別移除
-   **即時統計**：顯示已加入號碼的總數量

### 🚗 車位範圍管理

-   **自訂範圍**：可設定特定的車位範圍，而非固定範圍
-   **多範圍支援**：支援新增多個不重疊的車位範圍
-   **範圍顯示**：以標籤形式顯示已設定的車位範圍
-   **個別移除**：可個別移除特定的車位範圍

### 🚫 排除停車位管理

-   **預設排除**：身障車格（191-195）和充電機車位（313-322）預設排除
-   **靈活添加**：支援單一號碼或範圍格式
-   **視覺化管理**：標籤式顯示，可個別移除
-   **智慧排序**：自動排序和去重
...
-   **簡潔顯示**：抽籤號碼與停車位的直接對應
-   **統計資訊**：顯示抽籤號碼數量、可用車位數等摘要資訊
-   **CSV 匯出**：完整的抽籤資訊和結果
-   **號碼複製**：快速複製停車位號碼用於下輪排除
-   **歷史記錄**：自動保存最近 20 筆抽籤記錄，支援點擊查看詳細結果
-   **超額處理**：當抽籤號碼數量超過車位數量時，系統會隨機選擇部分號碼參與抽籤
